...
NS_OBJECT_ENSURE_REGISTERED (ChokeIpv6PacketFilter);

TypeId
ChokeIpv6PacketFilter::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ChokeIpv6PacketFilter")
    .SetParent<Ipv6PacketFilter> ()
    .SetGroupName ("Internet")
    .AddConstructor<ChokeIpv6PacketFilter> ()
    .AddAttribute ("Perturbation",
                   "The salt used as an additional input to the hash function of this filter",
                   UintegerValue (0),
                   MakeUintegerAccessor (&ChokeIpv6PacketFilter::m_perturbation),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}

ChokeIpv6PacketFilter::ChokeIpv6PacketFilter ()
{
  NS_LOG_FUNCTION (this);
}
...
int32_t
ChokeIpv6PacketFilter::DoClassify (Ptr< QueueDiscItem > item) const
{
  NS_LOG_FUNCTION (this << item);
  Ptr<Ipv6QueueDiscItem> ipv6Item = DynamicCast<Ipv6QueueDiscItem> (item);
