...
NS_OBJECT_ENSURE_REGISTERED (ChokeIpv4PacketFilter);

TypeId
ChokeIpv4PacketFilter::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ChokeIpv4PacketFilter")
    .SetParent<Ipv4PacketFilter> ()
    .SetGroupName ("Internet")
    .AddConstructor<ChokeIpv4PacketFilter> ()
    .AddAttribute ("Perturbation",
                   "The salt used as an additional input to the hash function of this filter",
                   UintegerValue (0),
                   MakeUintegerAccessor (&ChokeIpv4PacketFilter::m_perturbation),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}

ChokeIpv4PacketFilter::ChokeIpv4PacketFilter ()
{
  NS_LOG_FUNCTION (this);
}
...
int32_t
ChokeIpv4PacketFilter::DoClassify (Ptr<QueueDiscItem> item) const
{
  NS_LOG_FUNCTION (this << item);
  Ptr<Ipv4QueueDiscItem> ipv4Item = DynamicCast<Ipv4QueueDiscItem> (item);
